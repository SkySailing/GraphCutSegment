...
class GraphMaker:
    ...
    def __init__(self, filename='resource/dood.jpg', outfilename='output/output_dood.jpg'):
        self.image = None
        self.graph = None
        self.overlay = None
    ...
    def find_averages(self):
        ...
        # make the background seed postion to 0
        ...
        # make the frontground seed postion to 1
    ...
    def populate_graph(self):
        ...
        for (y, x), value in np.ndenumerate(self.graph):
            ...
            # each point connect the blow point and rihgt point
    ...
    def cut_graph(self):
        ...
        self.mask = np.zeros_like(self.image, dtype=bool)  # with 17W non-terminal nodes and 34W non-terminal edges.
        g = maxflow.Graph[float](len(self.nodes), len(self.edges))
        nodelist = g.add_nodes(len(self.nodes))
        #  The terminal edges are created with add_tedge.
        for node in self.nodes:
            g.add_tedge(nodelist[node[0]], node[1], node[2])
        # non-terminal edges are created with add_edge.
    ...
    def get_node_num(x, y, array_shape):
        ...
        :param y:
        :param array_shape:
        :return:the index in the whole img
        """
