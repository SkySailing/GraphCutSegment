...
class NewCutUI:
    def __init__(self, filename, outfilename):
        ...
        self.graph_maker = GraphMaker.GraphMaker(filename, outfilename)
    ...
    def on_open(self):
        ...
        if f is not None and f != "":
            self.graph_maker.load_image(str(f[0]))
            self.seedLabel.setPixmap(QPixmap.fromImage(
                self.get_qimage(self.graph_maker.get_image_with_overlay(self.graph_maker.seeds))))
            self.segmentLabel.setPixmap(QPixmap.fromImage(
                self.get_qimage(self.graph_maker.get_image_with_overlay(self.graph_maker.segmented))))
    ...
    def on_save(self):
        ...
        print('Saving')
        if f is not None and f != "":
            self.graph_maker.save_image(f[0])
...
if __name__ == "__main__":
    ...
    outputimgPath = 'output/output_dood.jpg'
    # filename = inputimgPath, outfilename = outputimgPath
    newUI = NewCutUI(filename = inputimgPath, outfilename = outputimgPath)
    newUI.run()
